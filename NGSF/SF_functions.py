...
def core(
    ...
    **kwargs,
    ...
    # sigma is NaN wherever the object does not cover the fitted range, and the
    # direct form dropped those wavelengths through the residual. Missing this
    # made every chi2 NaN for any spectrum narrower than the fit range.
    m_sigma = np.isfinite(sigma)

    gal_mask = m_gal.astype(np.float64)
    # obj and sigma are common to every pair, so fold them in on one side once.
    sn_mask = (m_sn & m_obj & m_sigma).astype(np.float64)
    ...
    # Number of wavelengths where object, sigma and both templates are finite.
    times = gal_mask @ sn_mask.T
    # A pair with no valid scaling had an all-NaN residual, so it counted zero.
    times = np.where(np.isfinite(b) & np.isfinite(d), times, 0.0)
    ...
    # Zero weight, not NaN, outside the object's coverage: the wavelength is
    # excluded, and a NaN here would propagate into every contraction.
    w = np.where(m_sigma, 1 / np.where(m_sigma, sigma, 1.0) ** 2, 0.0)
